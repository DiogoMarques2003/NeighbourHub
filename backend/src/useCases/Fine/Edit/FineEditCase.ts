...
export default class FineEditCase {
  constructor(
  ...
  async execute(data: IFineEditDTO): Promise<Fine> {
    const { amount, areaReservationId, commonAreaId, condominiumId, fineId, reason, userId } = data;
    ...
    if (condominium.adminId !== userId)
      throw new AppError('Somente o administrador do condomínio pode editar multas', 403);
    ...
    if (amount) fineDb.amount = amount;
    if (reason) fineDb.reason = reason;

    return this.fineRepository.update(fineDb);
  }
}
