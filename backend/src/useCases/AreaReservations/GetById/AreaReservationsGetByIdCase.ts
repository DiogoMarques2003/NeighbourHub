...
export default class AreaReservationsGetByIdCase {
  ...
  async execute(data: IAreaReservationsGetByIdDTO) {
    ...
    if (!address && condominium.adminId !== userId) throw new AppError('Não podes apagar esta reserva', 403);
    ...
    if (reservation.userId !== userId && condominium.adminId !== userId)
      throw new AppError('Não tens permissão para apagar esta reserva', 403);

    return this.areaReservationsRepository.findById(reservationId);
  }
}
